...
export default function HousePanel() {
  ...
  const presenceByDeviceId = useMemo(() => new Map(presence.map((item) => [item.device_id, item])), [presence]);
  ...
  const peopleCards = useMemo(
    () =>
      devices
        ...
        .sort((left, right) =>
          (left.device.owner_name || left.device.name).localeCompare(right.device.owner_name || right.device.name, 'hu')
        ),
    [devices, presenceByDeviceId]
  );

  const homeCount = peopleCards.filter((item) => item.presence?.current_state === 'home').length;
    ...
    <section className="overflow-hidden rounded-[2rem] border border-white/5 bg-[#0a0c10]/60 backdrop-blur-md transition-all">
      <div className="p-4 sm:px-6">
        <div className="flex items-start justify-between gap-3 sm:items-center">
          <button onClick={() => setIsExpanded((current) => !current)} className="flex items-center gap-3">
        ...
        <div className="mt-3 flex flex-col gap-2 sm:grid sm:grid-cols-[auto_1fr] sm:items-center">
          <div className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-center sm:w-fit">
            <span className="text-[11px] font-black uppercase tracking-widest text-emerald-400">{homeCount} otthon</span>
          </div>

          <button
            onClick={() => void fetchHouseData()}
            className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-white transition-transform active:scale-95 sm:w-auto sm:justify-self-end"
          >
            <Activity size={14} />
            Frissítés
          </button>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm text-slate-300">
                    Ez a panel mutatja, ki van itthon, melyik eszköz milyen állapotban van, és ide érkeznek a lakásban
                        ...
                        return (
                          <article
                            ...
                            className={`rounded-[2rem] border p-5 shadow-xl ${
                              isHome ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-white/10 bg-black/15'
                            }`}
                          >
                  ...
                  <div className="grid gap-6 xl:grid-cols-[1.3fr_0.9fr]">
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-400">
                            Itt tudod felvenni a figyelt telefonokat, lámpákat és egyéb otthoni eszközöket.
                          </p>
                        </div>
                        <button
                          onClick={resetForm}
                    ...
                    <div className="space-y-6">
                      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5">
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          A szenzor ezekre a slugokra fog mérési adatot küldeni. A nevet alapul véve automatikusan generáljuk.
                        </p>
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            ...
                            <select
                              ...
                              onChange={(event) =>
                                setForm((current) => ({
                                  ...current,
                                  device_kind: event.target.value as HouseDeviceRecord['device_kind'],
                                }))
                              }
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            <select
                              ...
                              onChange={(event) =>
                                setForm((current) => ({
                                  ...current,
                                  monitor_method: event.target.value as HouseDeviceRecord['monitor_method'],
                                }))
                              }
                              className="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white outline-none"
                            >
                      ...
                      <div className="rounded-[2rem] border border-white/10 bg-black/15 p-5">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-400">
                              Ide kerül majd minden megérkezés, lekapcsolás és fontos otthoni állapotváltozás.
                            </p>
                          </div>
                        </div>
