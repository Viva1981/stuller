...
export default function ShoppingList({ userName }: { userName: string }) {
  ...
  async function fetchItems() {
    ...
    if (data) {
      // Sorrend: is_special szerint (false elöl, true hátul)
      const sorted = data.sort((a, b) => Number(a.is_special) - Number(b.is_special))
      setItems(sorted)
    }
  }
  ...
  async function addItem() {
    if (!newItem) return
    const tempItem = { item: newItem, added_by: userName, is_special: isSpecial, is_completed: false }
    
    // Optimista frissítés: azonnal bekerül a listába vizuálisan
    setItems(prev => [...prev, { ...tempItem, id: 'temp' }].sort((a, b) => Number(a.is_special) - Number(b.is_special)))
  ...
  return (
    <div className="bg-[#0a0c10]/60 border border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-md transition-all">
      ...
      {/* COMPACT HEADER - Pontosan mint a Menü */}
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            {/* ÚJ TÉTEL RÖGZÍTÉSE (Ha aktív az isAdding) */}
            <AnimatePresence>
              {isAdding && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 p-3 rounded-2xl border border-amber-500/20 mb-4">
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-xl transition-all ${isSpecial ? 'text-amber-500 bg-amber-500/10' : 'text-slate-600'}`}
                    >
                      <Star size={16} fill={isSpecial ? "currentColor" : "none"} />
                    </button>
                    <button onClick={addItem} className="bg-amber-500 px-4 rounded-xl text-black text-[10px] font-black uppercase">Ok</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className={`font-bold text-sm ${item.is_completed ? 'line-through' : 'text-white'}`}>
                      {item.item}
                    </span>
