...
export default function FamilyCalendar({ currentUser }: { currentUser: FamilyUser }) {
  ...
  const [recurrence, setRecurrence] = useState<'none' | 'daily' | 'weekly' | 'workdays'>('none');
  ...
  const handleAddEvent = async (customTitle?: string, isDutyEvent: boolean = false) => {
    ...
    const eventData = {
      ...
      priority: isDutyEvent ? 'normal' : priority,
      is_duty: isDutyEvent,
      ...
      >
        ...
        <div className="absolute left-1.5 top-1.5 flex flex-col gap-0.5">
          {activeMemberColors.map((m) => (
            <div key={m.name} className={`w-2 h-2 rounded-full shadow-lg ${m.color}`} />
          ))}
        </div>
        {hasImportant && (
    ...
    <div className="space-y-4 max-w-4xl mx-auto pb-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`h-12 flex-1 flex flex-col items-center justify-center rounded-2xl border font-black text-[9px] transition-all uppercase ${
            filterMode === 'mine' ? 'bg-white text-black border-white' : 'bg-slate-900 text-slate-500 border-slate-800'
          }`}
        >
        ...
        <button
          ...
          className="h-12 w-12 flex items-center justify-center bg-slate-900 border border-slate-800 rounded-2xl text-slate-500 hover:text-red-400 transition-colors"
        >
          <X size={18} />
        </button>
      </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="space-y-3 mt-2">
            ...
            {getEventsForDate(selectedDate).length === 0 ? (
              ...
              getEventsForDate(selectedDate).map((e) => (
                <div
                  key={e.id}
                  className={`group p-4 rounded-3xl border flex items-center justify-between transition-all ${
                    e.is_duty
                      ? 'bg-blue-600/10 border-blue-500/20 shadow-inner'
                      : e.priority === 'fontos'
                        ? 'bg-red-600/10 border-red-500/20 shadow-lg'
                ...
                >
                  <div
                    ...
                    onClick={() => {
                      setEditId(e.id);
                      setTitle(e.title);
                      setSelectedMembers(e.member_names || []);
                      setTime(e.event_time.substring(0, 5));
                  ...
                  <button onClick={() => supabase.from('events').delete().eq('id', e.id).then(() => fetchEvents())} className="text-slate-600 hover:text-red-500 p-2 transition-colors">
                    <Trash2 size={18} />
                  </button>
                </div>
              ))
      ...
      <AnimatePresence>
        {showAddForm && (
          <motion.form
            ...
            className="bg-white p-6 rounded-[2.5rem] shadow-2xl text-black space-y-4 mt-2"
          >
            <input
              type="text"
              placeholder="Mi a program?"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              autoFocus
              className="w-full bg-slate-100 p-4 rounded-2xl text-lg font-bold outline-none border-none focus:ring-2 focus:ring-emerald-500 transition-all"
            />
            ...
            <div className="flex items-center gap-4">
              <button
                type="button"
                onClick={() => setPriority(priority === 'fontos' ? 'normal' : 'fontos')}
                className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-2xl font-black text-xs border-2 transition-all ${
                  priority === 'fontos' ? 'bg-red-600 border-red-600 text-white' : 'bg-white border-slate-100 text-slate-400'
                }`}
              >
